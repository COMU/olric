	...
	AnaPencere():QMainWindow()
	{
			...
			QString(), 1, 0 | QMessageBox::Default | QMessageBox::Escape) == 0)
		{
                 ...
                 //  QString cd=cdismi;   //komutta ismi yerleştirme????????????
                   ...
                   QProcess *proc1 ;          //=new QProcess;  //Qprocess p;p.start("dir \"documents\"");
                   // proc1 = new QProcess(this);
                    ...
                    proc1->addArgument("-l");
                    proc1->addArgument("-V");
                    proc1->addArgument("\"YOURVPN ISO9660\"");
                    proc1->addArgument("-v");
                    ...
                    if(proc1->start()) 
                     {
                         QMessageBox::information(this, QString::fromUtf8("sıkıstırlmıs dosya goruntusu olusturulamadı???????????????"));
                     }
                    ...
                    QProcess *proc2;
                      ...
                      proc2->addArgument("-J");
                      proc2->addArgument("-V"); 
                      proc2->addArgument("\"YOURVPN v0.2\""); 
                     ...
                     if(proc2->start())
                     {
                       QMessageBox::information(this, QString::fromUtf8("iso  oluruldu"));
                      }
